...
// Load balancing behavior when selecting an endpoint for the backend.
type LoadBalancing struct {
	// Consistently hash requests to backend endpoints based on a request property,
	// providing best-effort session affinity: requests with the same key are routed
	// to the same endpoint as long as the endpoint set is stable.
	ConsistentHash *ConsistentHash `json:"consistentHash"`
}

// The request property used as the hash key. Requests where the property is
// absent are not hashed and fall back to the default load balancing algorithm.
// +kubebuilder:validation:ExactlyOneOf=header;cookie;sourceIp;queryParam;expression
type ConsistentHash struct {
	...
	// Hash the value of an HTTP cookie. The cookie is never generated by the gateway.
	// +optional
	...
	// Hash the value of a query parameter.
	// +kubebuilder:validation:MinLength=1
	// +kubebuilder:validation:MaxLength=256
	// +optional
	...
	// for example `request.headers["x-user"]`.
	// +optional
