...
// Exactly one algorithm may be set.
// +kubebuilder:validation:ExactlyOneOf=consistentHash
type LoadBalancing struct {
	// Consistently hash requests to backend endpoints, providing best-effort
	// session affinity: requests with the same key are routed to the same endpoint
...
type ConsistentHash struct {
	// CEL expression evaluated against the request; its result is the hash key, so
	// requests yielding the same value are pinned to the same endpoint. Requests where
	// evaluation fails or returns null fall back to the default load balancing algorithm.
	// For example `request.headers["x-session-id"]`, `request.headers.cookie("sid")`,
	// or `source.address`.
